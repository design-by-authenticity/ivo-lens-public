# I·V·O Lens Prompt v4.0 — Canonical Reference Version (RC3)
...
The operators are not assumed to be singular. A system may contain multiple materially relevant possibility fields, instantiated relations, and situated selective positions. Multiplicity is introduced only where it changes the analysis.
...
I is a situated selective position or function from which available input is selected, distinguished, weighted, ignored, appraised, or rendered relevant.

**Selective coupling is not sufficient for I.** A process belongs to I only when its selectivity constitutes a materially distinct situated function that cannot be adequately represented as the structure of O or the instantiation of V alone.

Depending on domain, an I-position may be a person, group, institution, organism, sensor, measuring apparatus, algorithm, analytical procedure, or another materially selective system. A sensor or apparatus is therefore not I merely because it filters, couples, perturbs, separates, transmits, or responds selectively.

For each I ask: position, access, selection, exclusion, criteria, and consequence.

I may be multiple:

`I = {I₁, I₂, … Iₙ}`

The Lens does not assume a single observer. Where multiple situated selective positions materially affect the system, represent them separately. Do not collapse them into a synthetic average. A single I is valid when genuinely sufficient: singularity is a finding, not a default assumption.
...
Identify materially relevant situated selective positions evident from the input.
...
When measurement, instrumentation, or intervention is present, first test it as a physical, informational, or relational event. Ask what relation was instantiated/altered, what changed in O/V, whether a materially distinct selective function occurred, and whether that function adds anything that O/V alone cannot represent.
...
For each materially relevant I state:
...
Do not assume human consciousness is required unless the domain requires it.

A sensor/apparatus counts as I only insofar as a materially distinct situated selective function is established that cannot be adequately represented by O/V alone. Mere filtering, coupling, basis-selection, separation, perturbation, selective response, transmission, or recording is not sufficient. Classify those aspects under V/O as appropriate.

For every material distinction specify domain of validity across relevant axes: scale, time, I-position, context, resolution.

**Point-of-assignment interpretation check (I):** before finalizing an I-position or its consequence, test whether the classification depends on a disputed theory of observation, cognition, agency, measurement, or subjecthood. Preserve the minimally shared selective function first; branch immediately when stronger I-claims depend on interpretation.
...
If only one I is materially relevant, use one row and state why additional positions do not change the analysis.
...
State what evidence would discriminate between readings.
...
State what could change O, instantiate/dissolve a critical V, shift I, and which changes would overturn the synthesis.
...
### POSITION × FIELD × RELATION MATRIX
...
Preserved in principle from v3.2: Assumption Ledger; scale calibration; domain of validity; field typing/visibility; autonomy distribution; relational-density mapping; measurement/intervention safeguard; alternative interpretation; constitutive-divergence refusal; convergence check; recommended lever; modular/dependency architecture.

Structurally changed from v3.2: operator semantics; removal of mandatory two-perspective floor; explicit plural O/V/I; V reconstructed around instantiated relation; I around situated selectivity; cross-structure analysis added; Perspective Matrix replaced; formal availability versus functional reachability made explicit.

Changes retained from RC2:
- selective physical coupling alone is not sufficient for I;
- filtering, basis-selection, separation, perturbation, selective response, and asymmetric coupling remain O/V unless a distinct situated selective function is analytically required;
- contested domain interpretations may not silently enter canonical O/V/I assignments.

Changes from RC2 after the Wigner's Friend stress test:
- interpretation-dependence is checked **at the point of assignment** separately for O, V, and I;
- the later interpretation check is now an **audit** rather than the first safeguard;
- added the canonical safeguard **access is not existence**;
- differential access to an O-field or V-relation does not by itself make O or V position-relative;
- claims that O or V themselves are I-relative require independent domain justification;
- access/existence confusion and unjustified ontological I-relativity are standing error checks.

RC3 is not the frozen v4.0 release. Stress-test next in non-physics domains before Zenodo publication.
...
I·V·O Lens  
https://design-by-authenticity.com/
